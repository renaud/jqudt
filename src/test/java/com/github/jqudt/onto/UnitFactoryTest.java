...
public class UnitFactoryTest {
	...
	public void testGetUnit() throws Exception {
		...
		Assert.assertEquals("http://qudt.org/schema/qudt#TemperatureUnit", unit
				.getType().toString());
	}
	...
	public void testGetURIs() throws Exception {
		UnitFactory factory = UnitFactory.getInstance();
		List<String> units = factory
				.getURIs("http://qudt.org/schema/qudt#TemperatureUnit");
	...
	public void testGetOpenPHACTSUnit() throws Exception {
		UnitFactory factory = UnitFactory.getInstance();
		Unit unit = factory
				.getUnit("http://www.openphacts.org/units/Nanomolar");
		Assert.assertNotNull(unit);
		...
		Assert.assertEquals(0.000001, unit.getMultiplier().getMultiplier(),
				0.0000001);
		Assert.assertEquals(0, unit.getMultiplier().getOffset(), 0.01);
		Assert.assertEquals(
				"http://qudt.org/schema/qudt#MolarConcentrationUnit", unit
						.getType().toString());
	}
